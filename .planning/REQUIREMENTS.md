...
- [ ] **NAV-01**: `→` (right arrow) — within a "max per person" item, advance to next person; after last person, advance to next agenda item
- [ ] **NAV-02**: `Shift+→` — skip remaining people in current item, jump straight to next agenda item
- [ ] **NAV-03**: `←` (left arrow) — go to previous sub-state or previous item; symmetric with `→`
- [ ] **NAV-04**: `1`–`9` — jump to slide N (counts agenda items, not sub-states; jumping to an item lands on its first sub-state if it has any)
- [ ] **NAV-05**: Pressing `→` on the *last sub-state of the last agenda item* navigates to a dedicated "Meeting complete" slide (no further nav forward)
- [ ] **NAV-06**: All keyboard shortcuts early-return if `event.target` is `<input>`, `<textarea>`, or `isContentEditable` — keystrokes never escape edit-in-place inputs
...
- [ ] **THEME-01**: Light mode (default) and dark mode
- [ ] **THEME-02**: Toggle button visible in the UI (top-right of viewport)
- [ ] **THEME-03**: Theme preference persisted in localStorage
- [ ] **THEME-04**: No flash of wrong theme on reload — read theme from localStorage in an inline `<script>` in `<head>` before paint

### Persistence — `PERSIST`

- [ ] **PERSIST-01**: Auto-save full state to localStorage (key `meeting-manager.v1.state`) — `rawMarkdown`, parsed IR, current slide/sub-state index, per-slide timer states (`accumulatedMs`, `paused`, edited allocation), theme
- [ ] **PERSIST-02**: Debounce writes to ~500ms; force-flush on `pagehide` / `visibilitychange: hidden` / `beforeunload`
- [ ] **PERSIST-03**: On boot, restore state from localStorage if present; **always restore in paused state** (never resume running) per research finding on `performance.now()` cross-reload semantics
- [ ] **PERSIST-04**: Hash the agenda text on save — if a saved agenda has a different hash from the current one (or no saved agenda), reset slide index and timers but preserve theme + agenda text
- [ ] **PERSIST-05**: Wrap every localStorage call in try/catch; degrade to in-memory with a small banner if storage is unavailable (Firefox `file://`, Incognito, quota exceeded)
- [ ] **PERSIST-06**: Bounds-check restored slide/sub-state indices against the current agenda — clamp to valid range

### Lifecycle Guards — `GUARD`

- [ ] **GUARD-01**: `beforeunload` confirmation while any timer is running — browser-native "Leave site?" prompt
- [ ] **GUARD-02**: Disable the confirmation when all timers are paused (no false alarms on normal close)

### Wake Lock — `WAKE`

- [ ] **WAKE-01**: Call `navigator.wakeLock.request('screen')` when any timer transitions to running; release when all timers are paused or meeting ends
- [ ] **WAKE-02**: Handle re-acquisition on `visibilitychange: visible` if the lock was dropped (per MDN guidance)
- [ ] **WAKE-03**: Fail silently if the API is unsupported (older browser) — degrade is acceptable

### Sound — `SOUND`

- [ ] **SOUND-01**: Optional chime when a timer reaches 0 — a single short WebAudio beep
- [ ] **SOUND-02**: Sound off by default; toggleable via a button in the UI; preference persisted in localStorage
- [ ] **SOUND-03**: No sound for amber transitions, sub-state changes, or reset — only the zero-crossing chime

### Error Surfacing — `ERR`

- [ ] **ERR-01**: Inline parser error with line number when the markdown can't be parsed (e.g., "Line 4: missing duration after attribution")
- [ ] **ERR-02**: Inline parser error survives a partial parse — show successfully parsed items + an error banner about the bad lines (don't blank the screen)
- [ ] **ERR-03**: Friendly error if a dropped file isn't `.md` / `.markdown` / `.txt`

### Discord-Readiness Polish — `BRAND`

- [ ] **BRAND-01**: Minimum body font-weight 500; titles 700+ (Discord H.264 destroys thin fonts)
- [ ] **BRAND-02**: Minimum displayed font-size 24px on stage content
- [ ] **BRAND-03**: AAA contrast ratio on stage text (both light and dark themes)
- [ ] **BRAND-04**: OVERTIME banner has a thick stroke/outline so it survives 720p low-bitrate compression
- [ ] **BRAND-05**: Layout intact at 100%, 125%, and 150% browser zoom
