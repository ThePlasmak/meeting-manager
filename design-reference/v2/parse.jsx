...
(function () {
  ...
  function addMissingDeadlines(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Strip a dangling "by" / ", by" the user left behind (e.g. they deleted
      // the time but not the connector) so we don't end up with "…, by, by 1:14 PM".
      const cleaned = line.replace(/[,\s]*\bby\s*$/i, "").replace(/\s+$/, "");
