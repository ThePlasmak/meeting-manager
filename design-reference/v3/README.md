...
The ring's stroke-dasharray CSS transition is disabled while running (so per-frame rAF updates don't queue 360ms animations on every paint). When a bump / set-time / reset causes a remaining-ms jump > 200ms, the ring is animated imperatively via the Web Animations API (`circle.animate([{strokeDasharray: prev}, {strokeDasharray: next}], {duration: 360, easing: …})`) in a `useLayoutEffect`. This is necessary because driving the smoothing through React state inside `useEffect` lands AFTER the new dash has already been committed — the CSS transition has nothing to interpolate. The same pattern is used for `FlatTimer`'s bar width.
